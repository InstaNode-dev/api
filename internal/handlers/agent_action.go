...
// newAgentActionPromoteApprovalSent — returned in the 202 response body when
// a promote targeting non-development env triggers the email approval flow.
// Dev-env promotes bypass this code path entirely.
...
// AgentActionPromoteTokenExpired — GET /approve/:token returns this when the
// row's status is 'expired'. Re-using a stale link is impossible.
...
// AgentActionReadOnlySession — returned by RequireWritable middleware when a
// JWT carries read_only:true (admin impersonation session). Mutation is
// irrevocably disabled for the token's lifetime.
